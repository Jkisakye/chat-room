...
from database_service import verify_user, register, search_user, search_salt, save_salt
...
def login(user):
    userid = search_user(user.username)
    if userid == False:
        return userid
    else:
        salt = search_salt(userid) #the same salt will create the same hash if the input was the same as the user's password
        password = hash_password(user.password, salt) 
        return verify_user(user.username, password)
...
def register_user(user):
    if search_user(user.username): #checks if the username already exists and doesn't register
        ...
        hash, salt = create_hashed_password(user.password)
        userid = register(user.username, hash)
        save_salt(userid, salt)
        return True


#create a hash for the password on registration
def create_hashed_password(clear_text):
    clear_text = str.encode(clear_text) 
    salt = bcrypt.gensalt()
    hash = bcrypt.hashpw(clear_text, salt)

    return hash.decode('utf-8'), salt.decode('utf-8')


#create a hash for the input on login
def hash_password(input_text, salt):
    salt = str.encode(salt)
    input_text = str.encode(input_text)
    hash = bcrypt.hashpw(input_text, salt)
